...
export function fetchSongId(title) {
  return function (dispatch) {
    ...
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=' + process.env.MUSIXMATCH_API).then(
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=' + process.env.MUSIXMATCH_API).then(
    response => response.json(),
